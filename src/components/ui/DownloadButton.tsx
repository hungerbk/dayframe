    ...
    >
      ...
      {open && (
        <div className="absolute bottom-full mb-1.5 left-1/2 -translate-x-1/2 bg-white border border-border rounded-lg shadow-md overflow-hidden z-10 min-w-30">
          <button type="button" onClick={() => handleSelect("square")} className="w-full px-4 py-2 text-sm text-text text-left hover:bg-background transition-colors">
            정사각형
          </button>
          <button type="button" onClick={() => handleSelect("mobile")} className="w-full px-4 py-2 text-sm text-text text-left hover:bg-background transition-colors border-t border-border">
            모바일 (9:16)
          </button>
        </div>
      )}
