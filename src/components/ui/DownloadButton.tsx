  ...
  return (
    <div className={`relative ${isDownloading || disabled ? "cursor-not-allowed" : ""}`} {...anchorProps}>
      <button
        ...
        aria-expanded={open}
        className={`flex items-center h-10.5 gap-1.5 px-3 rounded-lg border border-border bg-background text-text text-sm font-medium transition-colors disabled:opacity-50 ${isDownloading || disabled ? "pointer-events-none" : "hover:bg-border/30 active:opacity-80"}`}>
